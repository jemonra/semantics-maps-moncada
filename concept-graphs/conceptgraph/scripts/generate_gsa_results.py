...
# This is needed for some imports in the Tag2Text files
...
GROUNDING_DINO_CHECKPOINT_PATH = os.path.join(
    GSA_PATH, "./groundingdino_swint_ogc.pth")
...
# Tag2Text checkpoint
TAG2TEXT_CHECKPOINT_PATH = os.path.join(
...
def process_tag_classes(text_prompt: str, add_classes: List[str] = [], remove_classes: List[str] = []) -> list[str]:
    ...
    for c in remove_classes:
        classes = [
            obj_class for obj_class in classes if c not in obj_class.lower()]
...
def main(args: argparse.Namespace):
    ...
    if args.class_set == "none":
        mask_generator = get_sam_mask_generator(args.sam_variant, SAM_DEVICE)
    else:
        sam_predictor = get_sam_predictor(args.sam_variant, SAM_DEVICE)
    ...
    if args.class_set == "scene":
        ...
        tagging_model = tagging_model.eval().to(TAGGING_MODEL_DEVICE)
    ...
    if args.save_video:
        video_save_path = args.dataset_root / \
            args.scene_id / f"gsa_vis_{save_name}.mp4"
        frames = []
    ...
    for idx in trange(len(dataset)):
        ...
        vis_save_path = color_path.parent.parent / \
            f"gsa_vis_{save_name}" / color_path.name
        detections_save_path = color_path.parent.parent / \
        ...
        if args.class_set in ["ram", "tag2text"]:
            ...
            raw_image = tagging_transform(raw_image).unsqueeze(
                0).to(TAGGING_MODEL_DEVICE)
        ...
        if args.class_set == "none":
            ...
            if args.detector == "dino":
                ...
                yolo_results_w_classes = yolo_model_w_classes.predict(
                    color_path)

                yolo_results_w_classes[0].save(
                    vis_save_path[:-4] + "_yolo_out.jpg")
                xyxy_tensor = yolo_results_w_classes[0].boxes.xyxy
                xyxy_np = xyxy_tensor.cpu().numpy()
                confidences = yolo_results_w_classes[0].boxes.conf.cpu(
                ).numpy()

                detections = sv.Detections(
                    xyxy=xyxy_np,
                    confidence=confidences,
                    class_id=yolo_results_w_classes[0].boxes.cls.cpu(
                    ).numpy().astype(int),
                    mask=None,
                )
