...
def draw_red_outline(image, mask):
    ...
    contours, _ = cv2.findContours(mask.astype(
        np.uint8) * 255, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def crop_image_and_mask(image: Image, mask: np.ndarray, x1: int, y1: int, x2: int, y2: int, padding: int = 0):
    ...
    if image.shape[:2] != mask.shape:
        print("Initial shape mismatch: Image shape {} != Mask shape {}".format(
    ...
    if image_crop.shape[:2] != mask_crop.shape:
        print("Cropped shape mismatch: Image crop shape {} != Mask crop shape {}".format(
...
def plot_images_with_captions(images, captions, confidences, low_confidences, masks, savedir, idx_obj):
    ...
    fig, axarr = plt.subplots(nrows, ncols, figsize=(
        10, 5 * nrows), squeeze=False)  # Adjusted figsize
    ...
    for i in range(n):
        ...
        if img_array.shape[:2] != masks[i].shape:
            ax.text(0.5, 0.5, "Plotting error: Shape mismatch between image and mask",
                    ha='center', va='center')
        else:
...
def extract_node_captions(args):
    ...
    chat = LLaVaChat(chat_args.model_path,
                     chat_args.conv_mode, chat_args.num_gpus)
    ...
    for idx_obj, obj in tqdm(enumerate(scene_map), total=len(scene_map)):
        ...
        for idx_det in tqdm(idx_most_conf):
            ...
            if args.masking_option == "blackout":
                image_crop_modified = blackout_nonmasked_area(
                    image_crop, mask_crop)
            ...
            image_tensor = chat.image_processor.preprocess(
                image_crop_modified, return_tensors="pt")["pixel_values"][0]

            image_features = chat.encode_image(
        ...
        if len(image_list) > 0:
            plot_images_with_captions(image_list, caption_list, confidences_list,
    ...
    for item in caption_dict_list:
        item["captions"] = [caption.replace(
            "The central object in the image is ", "") for caption in item["captions"]]
...
def refine_node_captions(args):
    ...
    for _i in trange(len(captions)):
        ...
        if elapsed_time > TIMEOUT:
            ...
            save_json_to_file(_dict, responses_savedir /
                              f"{_caption['id']}.json")
...
def build_scenegraph(args):
    ...
    indices_to_remove = [i for i in range(
        len(responses)) if object_tags[i].lower() in ["fail", "invalid"]]
    ...
    segment_ids_to_retain = [i for i in range(
        len(scene_map)) if i not in indices_to_remove]
    ...
    responses = [resp for resp in responses if resp['id']
                 in segment_ids_to_retain]
    ...
    if len(labels) != 0:
        ...
        for label in range(labels.max() + 1):
            ...
            subgraph = adjacency_matrix[component_indices][:,
                                                           component_indices]
        ...
        if not (Path(args.cachedir) / "cfslam_object_relations.json").exists():
            ...
            for componentidx, component in enumerate(components):
                if len(component) <= 1:
                    continue
                for u, v in zip(
                    minimum_spanning_trees[componentidx].nonzero(
                    )[0], minimum_spanning_trees[componentidx].nonzero()[1]
                ):
...
def annotate_scenegraph(args):
    ...
    if annot_inds is not None:
        annots = json.load(
            open(Path(args.cachedir) / "annotated_scenegraph.json", "r"))
    ...
    for idx in annot_inds:
        ...
        for idx_det in idx_most_conf:
            ...
            x1, y1, x2, y2 = xyxy
            image_crop = crop_image_pil(image, x1, y1, x2, y2, padding=padding)
            mask_crop = crop_image_pil(Image.fromarray(
                mask), x1, y1, x2, y2, padding=padding)
